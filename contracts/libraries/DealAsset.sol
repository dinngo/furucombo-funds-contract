...
library DealAsset {
    ...
    // conflict with the storage layout of the implementation behind the proxy.
    bytes32 internal constant _ASSET_ARR_SLOT =
    ...
    function get(address _key) internal view returns (bool) {
        ...
        return StorageMap.get(_ASSET_MAP_SLOT, key) == _ASSET_TRUE_FLAG;
    }
    ...
    function clean() internal {
        while (StorageArray.getLength(_ASSET_ARR_SLOT) > 0) {
            bytes32 key = StorageArray.pop(_ASSET_ARR_SLOT);
            StorageMap.set(_ASSET_MAP_SLOT, key, 0);
        }
    }

    function assets() internal view returns (address[] memory) {
        uint256 length = StorageArray.getLength(_ASSET_ARR_SLOT);
        address[] memory _assets = new address[](length);

        for (uint256 i = 0; i < length; i++) {
            _assets[i] = address(
                uint160(uint256(StorageArray.get(_ASSET_MAP_SLOT, i)))
            );
        }
        return _assets;
    }
