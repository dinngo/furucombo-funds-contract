...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (execAction === constants.AddressZero) {
    ...
    await (await comptrollerProxy.setExecAction(taskExecutor.address)).wait();
  }
  ...
  if (!canCall) {
    await (await comptrollerProxy.permitDelegateCalls(LEVEL, [aFurucombo.address], [WL_ANY_SIG])).wait();
  }

  // Transfer comptroller's ownership who can configures settings
  await (await comptrollerProxy.transferOwnership(MANAGEMENT)).wait();
  ...
  await (await comptrollerProxyAdmin.transferOwnership(MANAGEMENT)).wait();
  ...
  await (await beacon.transferOwnership(MANAGEMENT)).wait();
};
