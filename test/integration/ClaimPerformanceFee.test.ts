...
import { deployFurucomboProxyAndRegistry, createPoolProxyMock } from './deploy';
import {
  BAT_TOKEN,
  USDC_TOKEN,
  WETH_TOKEN,
...
describe('ManagerClaimPerformanceFee', function () {
  ...
  let poolProxy: PoolImplementationMock;
  ...
  const setupEachTestP0 = deployments.createFixture(
    async ({ deployments, ethers }, options) => {
      await deployments.fixture(''); // ensure you start from a fresh deployments

      await _preCreatePoolProxyMock();

      const pFeeRate = 0;
      // Create and finalize furucombo fund
      poolProxy = await createPoolProxyMock(
        poolProxyFactory,
        manager,
        denominationAddress,
        level,
      ...
      await _postCreatePoolProxyMock();
    }
  );

  const setupEachTestP1 = deployments.createFixture(
    async ({ deployments, ethers }, options) => {
      await deployments.fixture(''); // ensure you start from a fresh deployments
  ...
  const setupEachTestP99 = deployments.createFixture(
    async ({ deployments, ethers }, options) => {
      await deployments.fixture(''); // ensure you start from a fresh deployments
  ...
  describe('in operation', function () {
    ...
    describe('0% performance fee', function () {
      ...
      it('claim 0 fee when asset value declines', async function () {
        const pFee = await _assetValueDeclineTest(
          purchaseAmount,
          acceptPending
        );
      ...
      it('claim 0 fee when asset value is not changed (decline->grow back)', async function () {
        const pFee = await _assetValueNotChangedTestII(
          purchaseAmount,
          acceptPending
        );
        expect(pFee).to.be.eq(0);
      ...
      it('claim 0 fee when asset value grows more than last asset ATH value', async function () {
        const [beforeGrowShare, afterGrowShare] =
          await _assetValueHighWaterMarkTestII(purchaseAmount.div(2));
      ...
      it.skip('should revert when still in crystallization period', async function () {
        ...
        // claim pFee
        await expect(poolProxy.crystallize()).to.be.revertedWith(
  ...
  describe('in observation', function () {
    ...
    describe('99% performance fee', function () {
      ...
      it('claim fee when asset value grows + user partially redeem', async function () {
        const pFee = await _assetValueGrowTestII(
          initialFunds.sub(purchaseAmount),
          acceptPending
        );
      ...
      it('claim fee when asset value grows more than last asset ATH value', async function () {
        const [beforeGrowShare, afterGrowShare] =
          await _assetValueHighWaterMarkTestII(mwei('500'));
        expect(afterGrowShare).to.be.gt(beforeGrowShare);
        expect(await shareToken.balanceOf(outstandingAccount)).to.be.eq(0);
      });
      it('should revert when still in crystallization period', async function () {
  ...
  async function _preCreatePoolProxyMock() {
  ...
  async function _postCreatePoolProxyMock() {
  ...
  async function _assetValueDeclineTest(
    purchaseAmount: any,
    acceptPending: any
  ): Promise<any> {
    ...
    await poolProxy.setTotalAssetValueMock(mwei('1500'));
  ...
  async function _assetValueDeclineTestII(purchaseAmount: any): Promise<any> {
    ...
    const vaultAddr = await poolProxy.vault();
    await tokenA.connect(tokenAProvider).transfer(vaultAddr, ether('3000'));
    await poolProxy.connect(manager).addAsset(tokenAAddress);
  ...
  async function _unexpectedDenominationTest(
    purchaseAmount: any
  ): Promise<any> {
    await purchaseFund(
      investor,
  ...
  async function _assetValueNotChangedTest(
    purchaseAmount: any,
    acceptPending: any
  ): Promise<any> {
    const [share] = await purchaseFund(
  ...
  async function _assetValueNotChangedTestII(
    ...
    await poolProxy.setTotalAssetValueMock(1500);
  ...
  async function _assetValueGrowTest(): Promise<any> {
    ...
    // asset value grows
    await poolProxy.setTotalAssetValueMock(mwei('6000'));
  ...
  async function _assetValueHighWaterMarkTest(purchaseAmount: any) {
    ...
    await denomination
      .connect(denominationProvider)
      .transfer(vaultAddr, initialFunds);
    ...
    await poolProxy.setTotalAssetValueMock(mwei('1000'));
    ...
    await poolProxy.setTotalAssetValueMock(mwei('3000'));
  ...
  async function _assetValueHighWaterMarkTestII(
    purchaseAmount: any
  ): Promise<any> {
    await purchaseFund(
      investor,
  ...
  async function _crystallizationPeriodTest(
    purchaseAmount: any,
    acceptPending: any
  ) {
    const [share] = await purchaseFund(
