...
describe('Aave V2', function () {
  ...
  describe('Deposit', function () {
    ...
    describe('Matic', function () {
      ...
      it('max amount', async function () {
        const value = ether('10');
        const to = hAaveV2.address;
        const data = simpleEncode('depositETH(uint256)', [
  ...
  describe('Withdraw', function () {
    ...
    describe('Matic', function () {
      beforeEach(async function () {
        await wmatic
          .connect(wmaticProviderAddress)
          .approve(lendingPool.address, depositAmount);
        await lendingPool
          .connect(wmaticProviderAddress)
          .deposit(wmatic.address, depositAmount, user.address, 0);

        depositAmount = await awmatic.balanceOf(user.address);
      });

      it('partial', async function () {
        const value = depositAmount.div(BigNumber.from(2));
        const to = hAaveV2.address;
        const data = simpleEncode('withdrawETH(uint256)', [value]);
        await awmatic.connect(user).transfer(proxy.address, value);
        await proxy.updateTokenMock(awmatic.address);
        userBalance = await ethers.provider.getBalance(user.address);
      ...
      it('max amount', async function () {
        const value = depositAmount.div(BigNumber.from(2));
        const to = hAaveV2.address;
        const data = simpleEncode('withdrawETH(uint256)', [
          constants.MaxUint256,
        ]);
        await awmatic.connect(user).transfer(proxy.address, value);
        await proxy.updateTokenMock(awmatic.address);
        ...
        expectEqWithinBps(
          await balanceDelta(user.address, userBalance),
          value,
          100
        );
