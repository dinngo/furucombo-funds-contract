...
describe('Funds', function () {
  ...
  describe('update tokens', function () {
    ...
    it('native token - zero address', async function () {
      ...
      await token0.connect(provider0Address).transfer(proxy.address, value[0]);
      await sendEther(maticProviderAddress, proxy.address, value[1]);
      ...
      // Verify ether
      expect(handlerReturn[1]).to.be.eq(value[1].add(msgValue)); // handlerReturn should include msg.value
  ...
  describe('send', function () {
    ...
    describe('token', function () {
      it('normal', async function () {
        ...
        const value = ether('100');
        const receiver = someone.address;
        const to = hFunds.address;
        const data = simpleEncode('sendToken(address,uint256,address)', [
          token.address,
          value,
          receiver,
        ]);
        ...
        const tokenSomeone = await token.balanceOf(someone.address);
        const receipt = await proxy.connect(user).execMock(to, data, {
          value: ether('0.1'),
        });
        ...
        await expect(receipt)
          ...
          .withArgs(proxy.address, someone.address, value);

        const tokenSomeoneEnd = await token.balanceOf(someone.address);
        expect(tokenSomeoneEnd.sub(tokenSomeone)).to.be.eq(value);
        await profileGas(receipt);
      });
      ...
      it('USDT', async function () {
        ...
        await usdt.connect(providerAddress).transfer(proxy.address, value);
        await proxy.updateTokenMock(token.address);

        const tokenSomeone = await usdt.balanceOf(someone.address);
        const receipt = await proxy.connect(user).execMock(to, data, {
          value: ether('0.1'),
        });
        ...
        const tokenSomeoneEnd = await usdt.balanceOf(someone.address);
        expect(tokenSomeoneEnd.sub(tokenSomeone)).to.be.eq(value);
        await profileGas(receipt);
      });
      ...
      it('insufficient token', async function () {
        ...
        await token
          ...
          .transfer(proxy.address, value.sub(ether('1')));
        await proxy.updateTokenMock(token.address);
      ...
      it('should revert: not support MRC20', async function () {
        ...
        await proxy.updateTokenMock(token);
    ...
    describe('Ether', async function () {
      // let balanceSomeone: BigNumber;
      it('normal', async function () {
        const value = ether('1');
        const receiver = someone;
        const to = hFunds.address;
        const data = simpleEncode('send(uint256,address)', [
          value,
          receiver.address,
        ]);

        const balanceSomeone = await ethers.provider.getBalance(
          someone.address
        );
        ...
        expect(await balanceDelta(someone.address, balanceSomeone)).to.be.eq(
          value
        );
        await profileGas(receipt);
      });
      ...
      it('send 0 eth', async function () {
        ...
        const receiver = someone.address;
        const to = hFunds.address;
        const data = simpleEncode('send(uint256,address)', [value, receiver]);
        const balanceSomeone = await ethers.provider.getBalance(
          someone.address
        );
    ...
    describe('multiple tokens', function () {
      ...
      it('multiple tokens', async function () {
        ...
        const receiver = someone.address;
        const to = hFunds.address;
        const data = simpleEncode('sendTokens(address[],uint256[],address)', [
          tokens,
          value,
          receiver,
        ]);
        ...
        const token0Someone = await token0.balanceOf(someone.address);
        const token1Someone = await token1.balanceOf(someone.address);
        const receipt = await proxy.connect(user).execMock(to, data, {
          value: ether('0.1'),
        });
        ...
        await expect(receipt)
          .to.emit(token0, 'Transfer')
          .withArgs(proxy.address, someone.address, value[0]);

        await expect(receipt)
          .to.emit(token1, 'Transfer')
          .withArgs(proxy.address, someone.address, value[1]);

        const token0SomeoneEnd = await token0.balanceOf(someone.address);
        expect(token0SomeoneEnd.sub(token0Someone)).to.be.eq(value[0]);

        const token1SomeoneEnd = await token1.balanceOf(someone.address);
        expect(token1SomeoneEnd.sub(token1Someone)).to.be.eq(value[1]);
        await profileGas(receipt);
      });
  ...
  describe('check slippage', function () {
    ...
    it('should revert: not support MRC20', async function () {
      const token = [token0.address, constants.AddressZero, MATIC_TOKEN];
