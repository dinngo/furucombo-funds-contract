...
describe('SushiSwap Liquidity', function () {
  ...
  const setupTest = deployments.createFixture(async ({ deployments, ethers }, options) => {
    ...
    uniTokenMatic = await ethers.getContractAt('IERC20', sushiswapPoolAAddress);
    uniTokenToken = await ethers.getContractAt('IERC20', sushiswapPoolBAddress);
    router = await ethers.getContractAt('IUniswapV2Router02', sushiswapRouterAddress);

    await tokenA.connect(tokenAProvider).transfer(user.address, ether('1000'));
    await tokenB.connect(tokenBProvider).transfer(user.address, decimal6('1000'));
  ...
  describe('Add Token', function () {
    ...
    it('normal', async function () {
      ...
      expect(handlerReturn[2]).to.be.eq(uniTokenUserAmountEnd.sub(uniTokenUserAmount));
      ...
      // Verify spent ether
      expect(await uniTokenToken.balanceOf(user.address)).to.be.gt(uniTokenTokenUserAmount);
  ...
  describe('Remove Token', function () {
    ...
    beforeEach(async function () {
      ...
      await tokenB.connect(tokenBProvider).transfer(user.address, decimal6('100'));

      await tokenA.connect(user).approve(router.address, ether('1000'));
      const tx = await tokenB.connect(user).approve(router.address, decimal6('1000'));
    ...
    it('normal', async function () {
      ...
      await uniTokenToken.connect(user).approve(router.address, uniTokenUserAmount);
      ...
      await uniTokenToken.connect(user).transfer(proxy.address, uniTokenUserAmount);
