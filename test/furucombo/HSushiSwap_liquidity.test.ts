...
describe('SushiSwap Liquidity', function () {
  ...
  describe('Add Matic', function () {
    ...
    it.only('normal', async function () {
      ...
      const minMaticAmount = BigNumber.from('1');
      const value = ether('1');
      ...
      // Verify spent matic
      expect(await uniTokenMatic.balanceOf(user.address)).to.be.gt(
  ...
  describe('Remove Matic', function () {
    ...
    beforeEach(async function () {
      ...
      // deadline = (await latest()).add(BigNumber.from('100'));
    ...
    it('normal', async function () {
      ...
      await proxy.updateTokenMock(uniTokenMatic.address);
      ...
      expect(await uniTokenMatic.balanceOf(user.address)).to.be.eq(ether('0'));

      // Verify proxy token should be zero
      expect(await uniTokenMatic.balanceOf(proxy.address)).to.be.eq(ether('0'));
      expect(await tokenA.balanceOf(proxy.address)).to.be.eq(ether('0'));
      expect(await ethers.provider.getBalance(proxy.address)).to.be.eq(
        ether('0')
      );
