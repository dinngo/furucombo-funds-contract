...
describe('QuickSwap Liquidity', function () {
  ...
  describe('Add MATIC', function () {
    beforeEach(async function () {
      lpTokenUserAmount = await lpTokenMatic.balanceOf(user.address);
    });
    ...
    it('normal', async function () {
      ...
      const data = simpleEncode(
        ...
        [value, tokenAAddress, tokenAmount, minTokenAmount, minMaticAmount]
      );

      tokenAUserAmount = await tokenA.balanceOf(user.address);
      await tokenA.connect(user).transfer(proxy.address, tokenAmount);
      ...
      const lpTokenUserAmountEnd = await lpTokenMatic.balanceOf(user.address);
      ...
      expect(userBalanceDelta).to.be.eq(ether('0').sub(handlerReturn[1]));
      ...
      // Result Verification
      // Verify spent matic
      expect(userBalanceDelta).to.be.lte(ether('0').sub(minMaticAmount));
      ...
      // Verify spent matic
      expect(await lpTokenMatic.balanceOf(user.address)).to.be.gt(
    ...
    it('matic token', async function () {
      ...
      const data = simpleEncode(
        'addLiquidityETH(uint256,address,uint256,uint256,uint256)',
        [value, MATIC_TOKEN, tokenAmount, minTokenAmount, minMaticAmount]
      );
  ...
  describe('Remove MATIC', function () {
    ...
    beforeEach(async function () {
      ...
      const tx = await tokenA
        .connect(user)
        .approve(router.address, ether('0.01'));
    ...
    it('normal', async function () {
      ...
      await proxy.updateTokenMock(lpTokenMatic.address);

      const value = lpTokenUserAmount;
      const to = hQuickSwap.address;
      const data = simpleEncode(
        'removeLiquidityETH(address,uint256,uint256,uint256)',
        [tokenAAddress, value, BigNumber.from('1'), BigNumber.from('1')]
      );
      ...
      expect(userBalanceDelta).to.be.eq(handlerReturn[1]);

      // Verify User Token
      expect(await tokenA.balanceOf(user.address)).to.be.eq(
        tokenAUserAmount.add(result[0])
      );
      expect(await lpTokenMatic.balanceOf(user.address)).to.be.eq(ether('0'));

      // Verify proxy token should be zero
      expect(await lpTokenMatic.balanceOf(proxy.address)).to.be.eq(ether('0'));
      expect(await tokenA.balanceOf(proxy.address)).to.be.eq(ether('0'));
      expect(await ethers.provider.getBalance(proxy.address)).to.be.eq(
        ether('0')
      );
      ...
      // Verify spent matic
      expect(userBalanceDelta).to.be.eq(result[1]);
    ...
    it('matic token', async function () {
      ...
      const data = simpleEncode(
        'removeLiquidityETH(address,uint256,uint256,uint256)',
        [MATIC_TOKEN, value, BigNumber.from('1'), BigNumber.from('1')]
      );
