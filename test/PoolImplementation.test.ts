...
describe('PoolImplementation', function () {
  ...
  const pendingExpiration = ONE_DAY * 0.5; // 0.5 day
  ...
  describe('State changes', function () {
    ...
    describe('Finalize', function () {
      it('should success', async function () {
        ...
        const lastMFeeClaimTime = await poolImplementation.lastMFeeClaimTime();
        expect(lastMFeeClaimTime).to.be.eq(timestamp);

        // check performance fee initilize
        const lastGrossSharePrice =
          await poolImplementation.lastGrossSharePrice64x64();
        const hwm64x64 = await poolImplementation.hwm64x64();
