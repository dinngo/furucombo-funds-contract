...
describe('Chainlink', function () {
  ...
  describe('Add assets', function () {
    ...
    it('should revert: invalid length', async function () {
      await expect(chainlink.connect(owner).addAssets([], [aggregatorA])).to.be.revertedWith('RevertCode(40)'); // CHAINLINK_ASSETS_AND_AGGREGATORS_INCONSISTENT
    });

    it('should revert: zero address asset', async function () {
      await expect(
        chainlink.connect(owner).addAssets([constants.AddressZero], [aggregatorA])
      ).to.be.reverted.revertedWith('RevertCode(41)'); // CHAINLINK_ZERO_ADDRESS
    });
    ...
    it('should revert: stale price', async function () {
      ...
      await expect(chainlink.connect(owner).addAssets([tokenA], [aggregatorA])).to.be.revertedWith('RevertCode(45)'); // CHAINLINK_STALE_PRICE
    });
    ...
    it('should revert: invalid price', async function () {
      ...
      ).to.be.reverted.revertedWith('RevertCode(44)'); // CHAINLINK_INVALID_PRICE
    });
  });
  ...
  describe('Remove assets', function () {
    ...
    it('should revert: non-existent asset', async function () {
      await expect(chainlink.removeAssets([unsupportedToken])).to.be.revertedWith('RevertCode(43)'); // CHAINLINK_NON_EXISTENT_ASSET
    });
  });
  ...
  describe('Calculate quote amount', function () {
    ...
    it('should revert: stale price', async function () {
      ...
      await network.provider.send('evm_increaseTime', [stalePeriod.toNumber()]);
      await network.provider.send('evm_mine', []);
      await expect(chainlink.calcConversionAmount(base, baseAmount, quote)).to.be.revertedWith('RevertCode(45)'); // CHAINLINK_STALE_PRICE
    });
  });
});
